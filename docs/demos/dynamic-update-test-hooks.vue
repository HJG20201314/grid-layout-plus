<template>
  <div class="demo-container">
    <div class="controls">
      ...
      <button @click="updatePositionAndSize">更新位置和尺寸</button>
      <button @click="updatePositionOnly">仅更新位置</button>
      <button @click="updateSizeOnly">仅更新尺寸</button>
      <button @click="randomUpdate">随机更新</button>
      <button @click="resetToInitial">重置到初始值</button>
    </div>
    ...
    <div class="demo-area">
      ...
      >
        <div class="test-box">
          <div class="info">
            <p>位置: ({{ Math.round(api?.x.value ?? 0) }}, {{ Math.round(api?.y.value ?? 0) }})</p>
            <p>尺寸: {{ Math.round(api?.width.value ?? 0) }} × {{ Math.round(api?.height.value ?? 0) }}</p>
            <p class="tip">拖拽我或调整边缘试试！</p>
          </div>
        </div>
      </div>
    </div>
...
// 在 setup 中创建一次 hook，参考 DraggableResizableExample-hooks.vue
let api: UseDraggableResizableWrapperReturn = useDraggableResizableWrapper({
  draggable: true,
  resizable: true,
  // 与示例一致，使用 attach 绑定目标元素
  initialX: x,
  initialY: y,
  initialWidth: width,
  initialHeight: height,
  // 优化拖拽体验，仅从 .info 区域拖拽，避免误拖拽
  dragOptions: { allowFrom: '.info' },
  // 完整四边缩放
  resizeOptions: { edges: { top: true, right: true, bottom: true, left: true }, margin: 6 },
})

// 统一的 ref 绑定：仅执行 attach，不创建 hook
const bindWrapper = (elOrComp: Element | ComponentPublicInstance | null) => {
  if (!elOrComp) return
  const el = (elOrComp as any).$el ? (elOrComp as any).$el as Element : (elOrComp as Element)
  api.attachElement(el as HTMLElement)
}

const updatePositionAndSize = () => { api?.updatePositionAndSize(newX.value, newY.value, newWidth.value, newHeight.value) }
const updatePositionOnly = () => { api?.updatePosition(newX.value, newY.value) }
const updateSizeOnly = () => { api?.updateSize(newWidth.value, newHeight.value) }
...
const resetToInitial = () => { newX.value = 50; newY.value = 50; newWidth.value = 200; newHeight.value = 150; updatePositionAndSize() }
</script>
